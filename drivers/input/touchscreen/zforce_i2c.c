...
static int __zForce_read_data (struct i2c_client *client, char* buffer)
{
	...
	while (zForce_ir_touch_detect_int_level() && retry--) 
		schedule_timeout(2);
	
	if (!retry)
		printk (KERN_ERR "[%s-%d] timeout waiting int low !!\n",__func__,__LINE__);
...
static void zForce_ir_touch_report_data(struct i2c_client *client, uint8_t *buf)
{
	...
	if (2 == state) {
		...
		if (gIsCustomerUi) 
		{
			...
			input_report_abs(zForce_ir_touch_data.input, ABS_MT_POSITION_X, last_x);
			input_report_abs(zForce_ir_touch_data.input, ABS_MT_POSITION_Y, last_y);
		}
		g_touch_pressed = 0;
//		printk ("[%s-%d] touch up (%d, %d) (%02X, %02X, %02X)\n",__func__,__LINE__, last_x, last_y,buf[4], buf[5], buf[6]);
		...
		if (gIsCustomerUi) 
		{
			...
			input_report_abs(zForce_ir_touch_data.input, ABS_MT_POSITION_Y, x1);
			input_report_abs(zForce_ir_touch_data.input, ABS_MT_POSITION_X, y1);
			last_y = x1;
			last_x = y1;
...
//	enable_irq(zForce_ir_touch_data.client->irq);
	...
	set_bit(ABS_MT_TOUCH_MAJOR, zForce_ir_touch_data.input->absbit);
	set_bit(ABS_MT_POSITION_X, zForce_ir_touch_data.input->absbit);
	set_bit(ABS_MT_POSITION_Y, zForce_ir_touch_data.input->absbit);
	set_bit(ABS_MT_WIDTH_MAJOR, zForce_ir_touch_data.input->absbit);
